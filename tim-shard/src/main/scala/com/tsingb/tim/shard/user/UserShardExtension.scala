 ...
 * 用户Sharding
 */
object UserShardExtension extends ExtensionId[UserShardExt] with ExtensionIdProvider {

  override def get(system: ActorSystem) = super.get(system)
  override def lookup = UserShardExtension
  override def createExtension(system: ExtendedActorSystem) = new UserShardExt(system)

}

class UserShardExt(system: ExtendedActorSystem) extends Extension with Serializable {

  import ShardRegion.ShardId
  import UserShardExtension._
  ...
  private[this] val shardName = system.settings.config.getString("com.tsingb.tim.sharding.user-shard-name")
  private[this] val receiveOfflineMessage = system.settings.config.getBoolean("com.tsingb.tim.user.receive-offline-message")
  ...
  def start = ClusterSharding(system).start(
    typeName = shardName,
    entityProps = Props(classOf[UserShardActor], receiveOfflineMessage),
    settings = settings,
